const PLAN_LEVEL = "Beginner";
const PLAN_VERSION = "beginner-4week-daily-v1";
const STORAGE_KEY = "beginnerFlexibilityTracker.weekly4.v1";
const PLAN_WEEKS = 4;
...
function weekNoteRecord(id) {
  const weekNumber = Number(id.match(/^beginner4-week-(\d+)-notes$/)?.[1] || "");
...
function dayFromItemId(id) {
  ...
  const stableMatch = id.match(/^beginner4-week-(\d+)-([a-z0-9-]+)-(\d+)$/);
...
function migrateNoteId(id) {
  const currentMatch = id.match(/^beginner4-week-(\d+)-notes$/);
  if (currentMatch) {
...
function legacyWorkoutFromItemId(id) {
  const match = id.match(/^beginner4-week-(\d+)-day-(\d+)-workout-(\d+)$/);
...
function stableWorkoutId(weekNumber, workout) {
  return `beginner4-week-${weekNumber}-${slugify(workout.area)}-${workout.workoutNumber}`;
}
...
function itemId(weekNumber, dayIndex, workoutIndex) {
  ...
  return workout ? stableWorkoutId(weekNumber, workout) : `beginner4-week-${weekNumber}-day-${dayIndex}-workout-${workoutIndex}`;
}

function weekNoteId(weekNumber) {
  return `beginner4-week-${weekNumber}-notes`;
}
