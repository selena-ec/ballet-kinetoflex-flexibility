...
This Beginner tracker can save progress to Google Sheets through a small Google Apps Script Web App.
...
- Workout progress is stored in a `BeginnerWorkoutProgress` tab.
- Week notes are stored in the existing `BeginnerCycleNotes` tab.
- If an old `BeginnerTrackerState` tab exists, the script can still read it and the page will migrate matching progress into the row-based format.
